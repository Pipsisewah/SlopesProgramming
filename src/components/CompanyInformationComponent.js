...
const CompanyInformationComponent = ({company}) => {
    ...
    return (
             <div
                className={"max-w-2xl w-full grid grid-rows-3 grid-cols-8 gap-1 rounded-md p-4 shadow-md border border-white rounded"}
                key={company.id}>
                <p className={"col-span-4 font-semibold text-md text-left divide-y-1 pl-4"}>{company.name}</p>
